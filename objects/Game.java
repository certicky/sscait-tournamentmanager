...
import server.AchievementEngine;
...
public class Game implements Serializable
{
	...
	public boolean isHourTimeout() {
		return this.getFinalFrame() >= ServerSettings.Instance().tmSettings.GameFrameLimit;
	}

	/*
	 * Adds the game result to database (called when the game finishes).
	 */
	public synchronized void saveResultToDatabase() throws SQLException {
		...
		if (this.wasDraw || this.isHourTimeout()) {
			if (ServerSettings.Instance().BreakTies) {
				// if draws are forbidden, we need to resolve the situation based on in-game score...
				note += "draw;";
				ServerSettings.Instance().database.saveGameResult(this.gameID, "unfinished", note, this.hostScore, this.awayScore);
				ServerSettings.Instance().database.setResultByScore(this.gameID);
				// NOTE: we don't process achievements if this is just a draw resolved by score
			} else {
				// if draws are allowed, just add it to DB as a draw
				ServerSettings.Instance().database.saveGameResult(this.gameID, "draw", note, this.hostScore, this.awayScore);
				// process achievements if this isn't competitive phase
				if (!ServerSettings.Instance().CompetitivePhase) {
					AchievementEngine.Instance().processDraw(this.homebot);
					AchievementEngine.Instance().processDraw(this.awaybot);
				}
			}
		} else	if (this.hostwon) {
			// host won
			ServerSettings.Instance().database.saveGameResult(this.gameID, "1", note, this.hostScore, this.awayScore);
			// process achievements if this isn't competitive phase
			if (!ServerSettings.Instance().CompetitivePhase) {
				AchievementEngine.Instance().processWin(this.homebot);
				AchievementEngine.Instance().processLoss(this.awaybot);
			}
		} else {
			// guest won
			ServerSettings.Instance().database.saveGameResult(this.gameID, "2", note, this.hostScore, this.awayScore);
			// process achievements if this isn't competitive phase
			if (!ServerSettings.Instance().CompetitivePhase) {
				AchievementEngine.Instance().processLoss(this.homebot);
				AchievementEngine.Instance().processWin(this.awaybot);
			}
		}
